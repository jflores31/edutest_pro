...
- **Exam certificate ("constancia"):** **rendered server-side** from the `Constancia de Examen.dc.html` design — Django template `apps/exams/templates/exams/certificate.html`, context built by `services/certificate.py`, served by `GET /api/v1/attempts/{id}/certificate/` (`AttemptViewSet.certificate`). The endpoint enforces **completed + passed (`score ≥ PASS_THRESHOLD`) + same organization** — the "solo aprobados" rule lives in the backend, not the UI. **Access:** teacher/admin via session cookie (org-scoped by `get_queryset`); student via the short-lived `certificate` token from the submit response (passed only). The frontend just **fetches the HTML** (`attempts.certificate()` in `api.js`) and **prints it** (`utils/certificate.js` `printHtml` → hidden iframe → "Save as PDF"). Because it renders inside a same-origin iframe under the strict CSP, the template uses **inline styles + system fonts only** (no `<script>`, no external/Google fonts). Buttons: `StudentResultsPage` (only when `result.passed`) and `AttemptDetailPage` (only when `COMPLETED` + `isPassing(score)`).
